...
fp_octree::fp_octree()
{
	m_children = NULL;
	coplanar = false;
	m_centroid = Eigen::Vector4d(0, 0, 0, 0);
	color = Eigen::Vector4d(0.5, 0.5, 0.5);
	variance1 = variance2 = variance3 = 0.0;
	votes = 0;
}


void fp_octree::clear()
{
	if (m_children != NULL)
	{
		for (short i = 0; i < 8; i++)
...
void fp_octree::subdivide(fp_settings &settings)
{
...
void fp_octree::remove_outliers()
{
...
Eigen::Matrix<double, 3, 3> fp_octree::fast_covariance_matrix()
{
...
void fp_octree::least_variance_direction()
{
	...
	normal1 = Eigen::Vector4d(eigenvectors_matrix(0, min_index), eigenvectors_matrix(1, min_index), eigenvectors_matrix(2, min_index));
	normal2 = Eigen::Vector4d(eigenvectors_matrix(0, middle_index), eigenvectors_matrix(1, middle_index), eigenvectors_matrix(2, middle_index));
	normal3 = Eigen::Vector4d(eigenvectors_matrix(0, max_index), eigenvectors_matrix(1, max_index), eigenvectors_matrix(2, max_index));
}

double fp_octree::distance2plane(Eigen::Vector4d &point)
{
	return abs((point - m_centroid) & normal1.normalized);
}

void fp_octree::get_nodes(std::vector< fp_octree*> &nodes)
{
	if (m_children != NULL)
	{
		for (short i = 0; i < 8; i++)
...
void fp_octree::show(bool type, int height)
{
	height--;
	if (height == 0) {
		glColor3dv(color.data);
		if (type) {
			...
			if (color == Eigen::Vector4d(0.5, 0.5, 0.5)) {
				glutWireCube(m_size);
			}
			else {
